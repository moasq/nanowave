...
- `operation`: exactly `"build"`, `"edit"`, `"fix"`, or `"question"` — no other values. Use `"question"` when the user is asking a question, requesting information, or not requesting any app build/edit/fix action (e.g. "how many apps are there?", "what does this do?", "explain the architecture").
