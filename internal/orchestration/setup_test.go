...
func TestWriteInitialCLAUDEMDWatchOS(t *testing.T) {
	...
	data, err := os.ReadFile(filepath.Join(projectDir, ".claude", "CLAUDE.md"))
	if err != nil {
		t.Fatalf("failed to read CLAUDE.md: %v", err)
	}
	text := string(data)
	if !strings.Contains(text, "WatchApp") {
		t.Error("CLAUDE.md should contain the app name")
	}
	if !strings.Contains(text, "generic/platform=watchOS") {
		t.Error("CLAUDE.md should contain the watchOS build command")
	}
}
