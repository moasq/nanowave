...
- `platform_hint`: exactly `"ios"`, `"watchos"`, `"tvos"`, or `""` — no other values (primary platform for backward compat)
- `platform_hints`: array of platform strings, e.g. `["ios", "watchos", "tvos"]`. Each element must be `"ios"`, `"watchos"`, or `"tvos"`. Use `[]` when only one platform is targeted.
